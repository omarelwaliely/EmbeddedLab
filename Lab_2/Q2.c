...
void GPIOF_Handler(void)
{
 ...
 if (GPIOF->MIS == 0x01)
		{
			if (duty <= 0.85) //must be this because due to the division the duty cycle of greater than this will not work
			{
				duty += 0.05;
			}
		}
 if (GPIOF->MIS == 0x10)
		{
			if (duty >= 0.15) //must be this because due to the division the duty cycle of greater than this will not work
			{
				duty -= 0.05;
			}
		}
...
{
	if(flag){
		SysTick->LOAD = (15999*duty)/30 ;
		GPIOD->DATA |= 0x40;
	}
	else{
		SysTick->LOAD = (15999*((1-duty)))/30;
		GPIOD->DATA &= ~0x40;
	}
	flag =!flag;
}